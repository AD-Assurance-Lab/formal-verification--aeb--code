#!/usr/bin/env bash
# Build the AEB verification environment from nothing, and PROVE it works before
# returning. Ported from formal-verification--steering--code on 2026-09-04.
#
...
#  3. auto_LiRPA must come from upstream git with --no-deps: the PyPI release is stuck at
#     0.3 and fails on modern torch, and without --no-deps pip rewrites torch underneath
#     you. --no-deps then means its OWN import-time deps are missing, so appdirs, tqdm and
#     graphviz are installed separately (also --no-deps, so torch still cannot move).
#
...
# Matched to the steering study's environment of record, which is where the stack was
# actually measured (its certificates record torch/numpy at run time in their own _meta).
# This repo has no published bounds of its own yet, so it inherits rather than invents.
TORCH_VER=2.13.0
...
# every tool here selects its device through tools/gpu.py; if that cannot import, the
# silent-CPU defect is back and nothing will say so.
try:
    sys.path.insert(0, "tools")
    from gpu import require_cuda                                   # noqa: F401
    print("    tools/gpu.py require_cuda: OK")
except Exception as exc:
    fail.append(f"tools/gpu.py did not import: {type(exc).__name__}: {exc}")
